...
Cluster.prototype.updateClusterMarker = function () {
    ...
    var bnobj={gn:0,yn:0,rn:0,hn:0};
    for(var i=0;i<this._markers.length;i++)
    {
        if(this._markers[i].linkstate==false)
        {
            bnobj.hn++;
        }
        else
        {
            if(this._markers[i].prec>85)
            {
                bnobj.rn++;
            }
            else if(this._markers[i].prec>50)
            {
                bnobj.yn++;
            }
            else
    ...
    this._clusterMarker.setText(this._markers.length,bnobj);
